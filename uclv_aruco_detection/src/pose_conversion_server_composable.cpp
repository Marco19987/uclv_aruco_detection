...
namespace uclv_aruco_detection
{
  class PoseConversionServer : public rclcpp::Node
  {
  public:
    PoseConversionServer(const rclcpp::NodeOptions & options) : Node("pose_conversion_server", options)
    {
      // declare parameters
                                ...
                                std::shared_ptr<uclv_aruco_detection_interfaces::srv::PoseService::Response> response)
    {
      ...
      for (auto frame_name : frame_names)
      {
        frame_publishers_.push_back(
            this->create_publisher<geometry_msgs::msg::PoseStamped>(this->obj_name_ + "/" + frame_name + "/pose", qos));
      }
...
RCLCPP_COMPONENTS_REGISTER_NODE(uclv_aruco_detection::PoseConversionServer)